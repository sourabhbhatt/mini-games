...
export default function TestBaseOnline({
  playerName,
  gameRoomId,
  currentAction,
}) {
  ...
  useEffect(() => {
    const handleGameUpdate = (data) => {
      ...
      if (data.room && data.sentence) {
        const opponentName =
          data.room.players.find((player) => player.id !== socket.id)?.name ||
          "";
  ...
  const INPUT_TEXT_WORDS_ARRAY_OPPONENT =
    opponentText?.trim().split(/\s+/) || []; // ARRAY OF WORDS , OF OPPONENT'S TEXT
  ...
  useEffect(() => {
    if (timeElapsed === 30) {
      ...
      setOpponentResult(() =>
        calculateResults(givenSentence, opponentText, timeElapsed)
      );
  ...
  const broadcastHandleRestart = () =>
    socket.emit("restartGame", { gameRoomId: roomId });
  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center w-full h-full relative">
        {!isEditing && (
          <div className="absolute flex w-full justify-center p-4 top-5">
            <button onClick={onOpen} id="Scoreboard">
              <h1 className="text-violet-400/70 hover:text-violet-400 hover:underline hover:curor-pointer font-mono text-xl font-bold transition-all">
                Recent Scores &#x1F6C8;
              </h1>
            </button>
          </div>
        )}
        <h2 className="text-2xl text-center w-full px-4 font-bold">
          <span className="text-blue-500">{playerName}</span> vs{" "}
          <span className="text-red-500">{opponentName || "waiting.."}</span>
        </h2>
        {showResults ? (
          <div className="flex flex-col sm:flex-row gap-2 mt-56 md:mt-24">
            <MemoizedTypeTestResults
              data={result}
              handleRestart={broadcastHandleRestart}
              handleStartSame={setIsSameTest}
              type="player"
            />
            <MemoizedTypeTestResults
              data={opponentResult}
          ...
          <div
            className="relative test-base-area max-w-7xl z-30 hover:cursor-pointer"
            onClick={handleClick}
          >
            {!isEditing && (
              <div
                className={`absolute flex w-full text-md font-semibold tracking-widest h-full justify-center gap-2 items-center uppercase pt-14`}
              >
                {isGameStart ? (
                  <>{`Game starts in ${gameStartTimer}`}</>
                ) : (
                  <>
                    <BsCursorFill className="text-2xl" /> Click on this area to
                    continue
                  </>
                )}
              </div>
            ...
            <div
              className={`flex justify-center items-center w-full ${
                !isEditing ? "blur overflow-hidden" : ""
              }`}
            >
              {isEditing &&
                inputRef.current &&
              ...
              <div
                className={` flex flex-wrap content-start w-full text-slate-500 p-6 font-medium font-mono text-lg sm:text-3xl tracking-wide select-none scrollbar-hide z-40 snap-y ${
                  isEditing &&
                  inputRef.current &&
                  !inputRef.current.contains(document.activeElement)
              ...
              >
                <div className="flex w-full justify-between items-center max-w-7xl py-4">
                  <h2 className=" text-xl sm:text-2xl text-start w-full font-bold">
                    {INPUT_TEXT_WORDS_ARRAY[0] === ""
                      ? 0
                      : INPUT_TEXT_WORDS_ARRAY.length}{" "}
                    / {givenSentence?.split(" ").length}
                  </h2>
                  <h2 className=" text-xl sm:text-2xl text-end w-full font-bold">
                    {"00:"}
                    {timeElapsed < 10 ? `0${timeElapsed}` : timeElapsed}
                  </h2>
                </div>
                {givenSentence?.split(" ").map((word, wordIndex) => {
                  const EQUAL_INDEX_WORD_INPUT =
                    INPUT_TEXT_WORDS_ARRAY.at(wordIndex);
                  const EQUAL_INDEX_WORD_INPUT_OPPONENT =
                    INPUT_TEXT_WORDS_ARRAY_OPPONENT?.at(wordIndex); // Opponent
                  ...
                  return (
                    <span
                      key={`word_${word}_${wordIndex}`}
                      className={`border-red-500 ${
                        INPUT_TEXT_WORDS_ARRAY?.length > wordIndex + 1 &&
                        EQUAL_INDEX_WORD_INPUT !== defaultWord
                          ? "border-b-2"
                          : ""
                      }`}
                    >
                      {word.split("").map((letter, letterIndex) => {
                        const LETTER_ARRAY_OF_EQUAL_WORD =
                          EQUAL_INDEX_WORD_INPUT?.split("");
                        const LETTER_ARRAY_OF_EQUAL_WORD_OPPONENT =
                          EQUAL_INDEX_WORD_INPUT_OPPONENT?.split(""); // opponent
                        const InputLetterAtSameIndex =
                          LETTER_ARRAY_OF_EQUAL_WORD?.at(letterIndex);
                        return (
                          <span
                            key={`letter_${wordIndex}_${letterIndex}_${letter}`}
                            className={`${
                              InputLetterAtSameIndex !== undefined
                                ? InputLetterAtSameIndex ===
                                  defaultWord?.at(letterIndex)
                                  ? "text-green-500"
                                  : "text-red-500"
                                : ""
                            }
                      `}
                          >
                            ...
                            {isEditing &&
                              INPUT_TEXT_WORDS_ARRAY_OPPONENT?.length ===
                                wordIndex &&
                              letterIndex === 0 &&
                              opponentText.at(-1) === " " && (
                                <span className={`absolute mt-[3px]`}>
                            ...
                            {LETTER_ARRAY_OF_EQUAL_WORD?.at(letterIndex) !==
                              undefined &&
                              letterIndex ===
                                LETTER_ARRAY_OF_EQUAL_WORD.length - 1 &&
                              INPUT_TEXT_WORDS_ARRAY.length === wordIndex + 1 &&
                              isEditing &&
                              text.at(-1) !== " " && (
                                <span className={`absolute mt-[3px]`}>
                            ...
                            {LETTER_ARRAY_OF_EQUAL_WORD_OPPONENT?.at(
                              letterIndex
                            ) !== undefined &&
                              letterIndex ===
                                LETTER_ARRAY_OF_EQUAL_WORD_OPPONENT.length -
                                  1 &&
                              INPUT_TEXT_WORDS_ARRAY_OPPONENT.length ===
                                wordIndex + 1 &&
                              isEditing &&
